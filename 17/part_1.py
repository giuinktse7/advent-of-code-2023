...
def a_star(grid: np.ndarray, heuristic_fn: Callable[[Pos, float], float], start: Pos, end: Pos):
    ...
    while s.has_next():
        ...
        cached_consecutive = s.seen.get(key, np.inf)

        if cached_consecutive <= node.consecutive:
            continue

        s.seen[key] = min(cached_consecutive, node.consecutive)
        ...
        for pos, direction in neighbors:
            ...
            if s.seen.get(key, np.inf) < consecutive:
                continue
